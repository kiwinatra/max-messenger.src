...
public final class SequencesKt___SequencesKt$minus$4 implements Sequence<T> {
    ...
    public Iterator<T> iterator() {
        List<T> list = SequencesKt___SequencesKt.toList(this.$elements);
        return list.isEmpty() ? this.$this_minus.iterator() : SequencesKt___SequencesKt.filterNot(this.$this_minus, new SequencesKt___SequencesKt$minus$4$iterator$1(list)).iterator();
    }
}
