...
public final class SubSequence<T> implements Sequence<T>, DropTakeSequence<T> {
    ...
    public SubSequence(Sequence<? extends T> sequence2, int i, int i2) {
        ...
        this.endIndex = i2;
        if (i < 0) {
            throw new IllegalArgumentException(wj6.h(i, "startIndex should be non-negative, but is ").toString());
        } else if (i2 < 0) {
            throw new IllegalArgumentException(wj6.h(i2, "endIndex should be non-negative, but is ").toString());
        } else if (i2 < i) {
            throw new IllegalArgumentException(g63.h("endIndex should be not less than startIndex, but was ", i2, i, " < ").toString());
        }
    }
